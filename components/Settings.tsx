...
import { LogoutIcon, SpinnerIcon, UsersIcon, ClipboardIcon, CloudDownloadIcon, CheckCircleIcon, ExclamationCircleIcon, DownloadIcon, KeyIcon } from './icons';
    ...
    useEffect(() => {
        const fetchLatestVersion = async () => {
            ...
            try {
                ...
                const response = await fetch('https://api.github.com/repos/Preet3627/Attendance-Management-System/releases/latest', { headers });
                
                if (!response.ok) {
                    const rateLimitRemaining = response.headers.get('x-ratelimit-remaining');
                    if (rateLimitRemaining === '0') {
                        throw new Error('GitHub API rate limit exceeded. Add a personal access token to resolve this.');
                    }
            ...
            <div className={`mt-2 p-4 rounded-md ${
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        {status === 'error' && <p className="text-xs text-red-700">{errorMessage || 'Please check your internet connection.'}</p>}
                    </div>
                </div>
            </div>
...
const Settings: React.FC<SettingsProps> = ({ onSaveKey, onLogout, secretKey: initialKey, initialSetup = false, currentUser }) => {
    ...
    const [githubApiKey, setGithubApiKey] = useState(() => localStorage.getItem('GITHUB_API_KEY') || '');
    const [isSavingGithubKey, setIsSavingGithubKey] = useState(false);
    const [githubKeyMessage, setGithubKeyMessage] = useState('');
    ...
    const handleSaveGithubKey = () => {
        setIsSavingGithubKey(true);
        localStorage.setItem('GITHUB_API_KEY', githubApiKey);
        setTimeout(() => {
            setIsSavingGithubKey(false);
            setGithubKeyMessage('GitHub API Key saved successfully! The page will now reload to apply the changes.');
            ...
            {currentUser.role === 'superuser' && (
                ...
                <div className="p-6 bg-white rounded-lg shadow-lg space-y-6">
                    <h3 className="text-lg font-semibold text-slate-800 border-b pb-3 flex items-center gap-2">
                        <KeyIcon className="w-5 h-5" /> GitHub API Key
                    </h3>
                    <div className="bg-blue-100 border-l-4 border-blue-500 text-blue-800 p-4" role="alert">
                        <p className="font-bold">Avoid API Rate Limits</p>
                        <p className="mt-1 text-sm">
                            The app checks GitHub for updates. To prevent being rate-limited by GitHub's API, you can provide a personal access token. This is optional but recommended.
                            You can generate a token from your GitHub account's "Developer settings". No special permissions are required.
                        </p>
                    </div>
                    <div className="space-y-2">
                        <label htmlFor="github-api-key" className="block text-sm font-medium text-slate-700">GitHub Personal Access Token</label>
                        <div className="flex gap-4">
                            <input
                                type="password"
                                id="github-api-key"
                            ...
                            <button
                                ...
                                disabled={isSavingGithubKey}
                                className="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-700 hover:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 disabled:bg-indigo-500 disabled:cursor-wait"
                            >
                                {isSavingGithubKey ? <SpinnerIcon className="w-5 h-5" /> : 'Save'}
                            </button>
                        </div>
                            {githubKeyMessage && <p className="text-sm text-green-600 mt-2">{githubKeyMessage}</p>}
                    </div>
                </div>
                </>
            )}
